...
class ARGS:
    ...
    def generate_step(self, mout, input_ids, pre_screen_beam_width=40, weight=0., method="greedy", temperature=0.7, rm_cached=None, debug=True):
        out_logits = mout.logits[:, -1]
        if debug: print(f"{out_logits.shape=}")
        if debug: print(f"{pre_screen_beam_width=}")
        ...
        inputs = self.LLM.prepare_inputs_for_generation(input_ids=flat_trme.to(self.rm_dev), attention_mask=create_attention_mask(flat_trme.shape[1], flat_trme.shape[0]).to(self.rm_dev), past_key_values=None, use_cache=True)
        rm_out = self.RM(**inputs)
    ...
    def generate(self, prompt, weight=0., topk=1, max_new_token=128, method="greedy", temperature=0.7, chunk_size=5, debug=True):
        ...
        for word_idx in range(max_new_token):
